...
/* C wrapper by CZhao */
extern "C" void *cuboid_init(int *u) {
  return new Cuboid(u);
}

extern "C" void cuboid_destroy(void *c) {
  delete static_cast<Cuboid*>(c);
}

extern "C" void cuboid_transform(void *c, double x1, double x2,
    double x3, double *r1, double *r2, double *r3) {
  double y[3];
  static_cast<Cuboid*>(c)->Transform(x1, x2, x3, y[0], y[1], y[2]);
  *r1 = y[0];
  *r2 = y[1];
  *r3 = y[2];
}
